...
package object hw07 extends Homework07 {
  ...
  */
  def interp(KXCFAE: KXCFAE, env: Env, denv: Env, k: Cont): KXCFAEValue = KXCFAE match {
    ...
    case Id(name) => {
      if (name=="throw") k(lookup(name, denv))
      else if (name=="pThrowV") k(ThrowV())
      else k(lookup(name, env))
    }
    case Fun(params, body) => k(CloV(params, body, env))
    case App(f, args) =>                                   
      interp(f, env, denv, fv => {
                        help(List(), args, env, denv, fv, k)
      })
    case Withcc(name, body) => 
      {
        if(name=="throw") interp(body, env, denv+(name->ContV(k)), k)
        else interp(body, env+(name->ContV(k)), denv, k)
      }
    case Try(tryE, catchE) => 
      interp(Withcc("throw", tryE), env, denv, tryV => 
        if(tryV==ThrowV()) interp(catchE, env, denv, catchV => k(catchV))
        else k(tryV)
      )
    case Throw => interp(App(Id("throw"), List(Id("pThrowV"))), env, denv, k)
    case If0(cond, thenE, elseE) => 
      interp(cond, env, denv, condV => 
                              if(condV==ThrowV()) k(ThrowV())
                              else if(condV==NumV(0)) interp(thenE, env, denv, k)
                              else interp(elseE, env, denv, k)
      )
      
  }

  def help(previousValueList: List[KXCFAEValue], remainExprList: List[KXCFAE], env: Env, denv: Env, fv: KXCFAEValue, k: Cont): KXCFAEValue = remainExprList match {
    case e::rest => interp(e, env, denv, ev => {
                                        val newPVL = previousValueList :+ ev   // list 끝에 ev 넣기
                                        help(newPVL, rest, env, denv, fv, k)
                                      
    })
    case Nil => fv match {
          case CloV(params, body, fenv) => 
            if (params.size != previousValueList.size ) error(s"wrong arity")
            interp(body, fenv ++ (params zip previousValueList toMap ), denv, k)
          case ContV(kv) => 
  ...
  def run(str: String): String = eval(interp(KXCFAE(str), Map(), Map(), x=>x))
